<template>
  <div>
    ...
    <div
      bp="grid 12 vertical-center"
      class="matches-block-loading"
      v-if="noMatches"
    >
        ...
        >
          ...
          <div bp="6" v-if="match.isLigueShow === 'true'">
            <div bp="grid vertical-center">
              <img
                bp="3 hide show@md"
                class="ligue-one-show__img"
                src="/assets/club-logos/football.svg"
                alt="Ligue 1 Show logo"
              />
              <h2 bp="9@md" class="text-left">{{ match.title }}</h2>
            </div>
          </div>
          <div bp="3" class="demo">
            <h2>{{ parseTime(match.start) }}</h2>
          </div>
    ...
    <div class="container container-footer">
      <div bp="grid 6">
        ...
        <div class="text-right">
          <h2><router-link :to="nextLink">NEXT</router-link></h2>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  data() {
    return {
      today: format(
        parseISO(this.$route.params.date || new Date().toISOString()),
        "yyyy-MM-dd"
      ),
      dayName: format(
        parseISO(this.$route.params.date || new Date().toISOString()),
  ...
  mounted() {
    fetch(
      `https://svbiszik4b.execute-api.us-west-2.amazonaws.com/ligue1/schedule/${this
        .$route.params.date || format(new Date(), "yyyy-MM-dd")}`,
      {
        mode: "cors",
        headers: {
          "Content-Type": "application/json",
          "x-timezone-offset": new Intl.DateTimeFormat().resolvedOptions()
            .timeZone
        }
      }
    )
  ...
  watch: {
    $route(to) {
      ...
      this.prevLink = format(
        addDays(parseISO(to.params.date), -1),
        "yyyy-MM-dd"
      );
      this.nextLink = format(
        addDays(parseISO(to.params.date), 1),
        "yyyy-MM-dd"
      );
      fetch(
        `https://svbiszik4b.execute-api.us-west-2.amazonaws.com/ligue1/schedule/${to.params.date}`,
        {
          mode: "cors",
          headers: {
            "Content-Type": "application/json",
